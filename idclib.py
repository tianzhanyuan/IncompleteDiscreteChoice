...
class BipartiteGraph:
    ...
    def smallest_CDC(self):
        ...
        * ``bipartite == 0`` for outcome vertices (Y)
        * ``bipartite == 1`` for support/latent vertices (U)
        ...
        * If ``B`` is disconnected, the CDC additionally contains the implicit-equality
        ...
        if len(comps) > 1:
            for C in comps:
                y_part = tuple(sorted(v for v in C if B.nodes[v].get('bipartite') == 0))
                if y_part:
                    CDC.add(y_part)

        # Within each component, compute the critical sets using Algorithm 3
        for C in comps:
            YC = sorted(v for v in C if B.nodes[v].get('bipartite') == 0)
            UC = sorted(v for v in C if B.nodes[v].get('bipartite') == 1)
            ...
            add = BipartiteGraph.get_nr_sets(C_graph, Y_renamed, U_renamed)
    ...
    @staticmethod
    def G_min(A, B, Y, U):
        G_inv_c = BipartiteGraph.G_inv(set(Y) - set(A), B)
    ...
    def check_connectivity_complement(A, B, Y):
        A_c = set(Y) - set(A)
        G_inv_c = BipartiteGraph.G_inv(A_c, B)
    ...
    def check_connectivity_self(A, B, Y, U):
        G_min_set = BipartiteGraph.G_min(A, B, Y, U)
    ...
    @staticmethod
    def kids(A, B, Y, U):
        UA = set(BipartiteGraph.G_inv(A, B)) - set(BipartiteGraph.G_min(A, B, Y, U))
        C = [sorted(set(A) | set(BipartiteGraph.G(u, B))) for u in UA]
        return C

    @staticmethod
    def min_nr_supersets(A, B, Y, U):
        A = list(A)
        C_list = BipartiteGraph.kids(A, B, Y, U)

        CC_list = []
        for C in C_list:
            C_c = set(Y) - set(C)
            G_inv_c = BipartiteGraph.G_inv(C_c, B)
            ...
            if len(list_of_comps_sub) == 1:
                ...
                CC_list.extend(BipartiteGraph.get_unions(C, list_of_comps_sub, Y))
    ...
    def get_nr_sets_iteration(Q, R, B, Y, U):
        ...
        # Replacing Threads with a simple loop for stability 
        # (Unless your B is massive, this is usually faster in Python)
        for A in Q:
            supsets = BipartiteGraph.min_nr_supersets(A, B, Y, U)
            to_add = supsets - R_new # Use R_new here
            
            for subset in to_add:
                Q_new.add(tuple(subset))
                R_new.add(tuple(subset)) # Ensure everything in R is a tuple for consistency
    ...
    def get_nr_sets(B, Y, U):
        Q = set()
        A_list = [BipartiteGraph.G(u, B) for u in U]
        to_keep = [BipartiteGraph.check_connectivity_complement(A, B, Y) for A in A_list]
        ...
        while Q:
            Q, R = BipartiteGraph.get_nr_sets_iteration(Q, R, B, Y, U)
        res = R - {tuple(Y)}
        return res
